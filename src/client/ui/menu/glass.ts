...
export function paintGlassOverlay(
  ...
  for (const [cx, cy] of corners) {
    ...
    // frost fleck near the corner
    for (let i = 0; i < 14; i++) {
      const a = rand() * Math.PI * 2;
      const d = rand() * rand() * r * 0.55;
      ctx.fillStyle = shade(C.boneDim, 1, 0.03 + rand() * 0.05);
      ctx.beginPath();
  ...
  for (let i = 0; i < count; i++) {
    const x = rand() * w;
    const y = rand() * rand() * h; // gravity thins the lower pane
    ...
    ctx.stroke();
    // the glint, facing the flame
    const glintWarm = dist < m * 0.42;
    ctx.fillStyle = glintWarm
      ? shade(C.flameHi, 1, 0.35 + rand() * 0.3)
      : shade(C.parchment, 1, 0.22 + rand() * 0.2);
    ctx.beginPath();
