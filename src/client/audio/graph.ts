...
interface ScoreDef {
  ...
  crackle: boolean;
  bellSend: number;
  /** Per-biome INSTRUMENTATION (D94, operator: "I hear the same music in
   *  all levels") — different notes on identical instruments read as the
   *  same piece; the ear must know WHERE it is from the timbre alone. */
  melVoice: "box" | "low" | "water" | "horn";
  padStyle: "soft" | "dark" | "sea" | "organ";
  bassPulse: boolean;
}
...
const BIOME_SCORES: readonly ScoreDef[] = [
  { // Tallow Halls — the vigil's own lament, thinned: alone with a candle
    ...
    toll: { freq: 329.63, everyBars: 6 }, hush: { freq: 200, gain: 0.5 }, shimmer: false, crackle: false, bellSend: 0.8, melVoice: "box", padStyle: "soft", bassPulse: false,
  },
  { // Root Cellars — D-minor lament, earth-heavy, the town far above
    ...
    restEvery: 2, toll: { freq: 293.66, everyBars: 7 }, hush: { freq: 170, gain: 0.5 }, shimmer: false, crackle: false, bellSend: 0.9, melVoice: "low", padStyle: "dark", bassPulse: false,
  },
  { // Drowned Stacks — E phrygian, the half-step sigh of despair
    ...
    restEvery: 2, toll: { freq: 329.63, everyBars: 6 }, hush: { freq: 260, gain: 0.55 }, shimmer: false, crackle: false, bellSend: 1.0, melVoice: "water", padStyle: "sea", bassPulse: false,
  },
  { // Glassblack Furnaces — F minor, almost no song left, heat and dread
    ...
    restEvery: 3, toll: { freq: 174.61, everyBars: 8 }, hush: { freq: 120, gain: 0.55 }, shimmer: false, crackle: false, bellSend: 0.7, melVoice: "horn", padStyle: "dark", bassPulse: true,
  },
  { // Hollow Choir — broken love: HER tune returns, fullest of the floors
    ...
    toll: { freq: 440.0, everyBars: 5 }, hush: { freq: 300, gain: 0.4 }, shimmer: true, crackle: false, bellSend: 1.1, melVoice: "box", padStyle: "organ", bassPulse: false,
  },
  { // Wickless Deep — loneliness itself: no chords, single far notes
    ...
    restEvery: 2, toll: null, hush: { freq: 150, gain: 0.4 }, shimmer: false, crackle: false, bellSend: 1.3, melVoice: "box", padStyle: "soft", bassPulse: false,
  },
  { // The Bottom — the lament turns: Am F C G, the tune climbs home to her
    ...
    restEvery: 3, toll: { freq: 440.0, everyBars: 5 }, hush: { freq: 220, gain: 0.45 }, shimmer: true, crackle: false, bellSend: 0.9, melVoice: "box", padStyle: "organ", bassPulse: false,
  },
];
...
export class AudioGraph {
  ...
  /** One pad voice, styled per biome (D94): "soft" = breathing sine+tri
   *  (the vigil); "dark" = triangle pair under a low ceiling (earth);
   *  "sea" = soft + a seasick pitch heave (water); "organ" = straight
   *  octave+twelfth stack, no breath (the Choir's dead pipes). */
  private themePadVoice(
    freq: number,
    pan: number,
    t0: number,
    holdS: number,
    peak: number,
    style: ScoreDef["padStyle"],
  ): void {
    const nodes: AudioNode[] = [];
    const oscs: OscillatorNode[] = [];
    const mkOsc = (type: OscillatorType, f: number, cents: number): OscillatorNode => {
      const o = this.ctx.createOscillator();
      o.type = type;
      o.frequency.value = f;
    ...
    lp.type = "lowpass";
    lp.frequency.value =
      style === "dark" ? Math.min(900, freq * 3.5) : style === "organ" ? 2200 : Math.min(1400, freq * 6);
    mkOsc(style === "dark" ? "triangle" : "sine", freq, 0).connect(lp);
    const trimB = this.ctx.createGain();
    if (style === "organ") {
      // the octave rank, straight as stone; a whisper of the twelfth
      trimB.gain.value = 0.4;
      mkOsc("sine", freq * 2, 2).connect(trimB);
      const trimC = this.ctx.createGain();
      trimC.gain.value = 0.16;
      mkOsc("sine", freq * 3, -3).connect(trimC);
      trimC.connect(lp);
      nodes.push(trimC);
    } else {
      trimB.gain.value = 0.35;
      mkOsc("triangle", freq, 4 + Math.random() * 4).connect(trimB);
    }
    trimB.connect(lp);
    ...
    if (style !== "organ") {
      ...
      breathDepth.gain.value = peak * (style === "sea" ? 0.22 : 0.16);
    ...
    if (style === "sea") {
      // seasick: the whole voice slowly heaves a few cents
      const wob = this.ctx.createOscillator();
      wob.frequency.value = 0.13 + Math.random() * 0.05;
      const wobDepth = this.ctx.createGain();
      wobDepth.gain.value = 5; // cents
      wob.connect(wobDepth);
      wobDepth.connect(oscs[0]!.detune);
      wobDepth.connect(oscs[1]!.detune);
      oscs.push(wob);
      nodes.push(wobDepth);
    }
    ...
    nodes.push(lp, trimB, env, panner, send);
  ...
  /** The lament bass — dry, centered, felt in the floor. The Furnaces
   *  THROB: a slow forge-pulse kneads the bass gain (D94). */
  private themeBass(freq: number, t0: number, holdS: number, pulse: boolean): void {
    ...
    let throb: OscillatorNode | null = null;
    let throbDepth: GainNode | null = null;
    if (pulse) {
      throb = this.ctx.createOscillator();
      throb.frequency.value = 0.55;
      throbDepth = this.ctx.createGain();
      throbDepth.gain.value = 0.028;
      throb.connect(throbDepth);
      throbDepth.connect(env.gain);
      throb.start(t0);
      throb.stop(t0 + holdS + 3.1);
    }
    osc.start(t0);
    ...
    osc.onended = () => {
      ...
      env.disconnect();
      throb?.disconnect();
      throbDepth?.disconnect();
    };
  }

  /** The melody voice (D94): "box" = the music-box (partials 3.01/5.4);
   *  "low" = the same box an octave down, slower ring (earth); "water" =
   *  wide drift + a sinking glide on every partial (drowned); "horn" = no
   *  bell at all — a mournful lowpassed horn swell (the Furnaces). */
  private themeBell(
    freq: number,
    atS: number,
    peak: number,
    ...
    if (voice === "horn") {
      // a horn heard through furnace walls — swell, ache, die
      const osc = this.ctx.createOscillator();
      osc.type = "sawtooth";
      ...
      const env = this.ctx.createGain();
      env.gain.setValueAtTime(SILENT, t0);
      env.gain.linearRampToValueAtTime(peak * 0.9, t0 + 0.5);
      env.gain.setValueAtTime(peak * 0.9, t0 + dur * 0.55);
      env.gain.exponentialRampToValueAtTime(SILENT, t0 + dur * 1.3);
      osc.connect(lp).connect(env).connect(panner);
      osc.start(t0);
      osc.stop(t0 + dur * 1.3 + 0.05);
      osc.onended = () => {
        for (const n of [osc, lp, env, panner, dry, send]) n.disconnect();
      };
      return;
    }
    ...
    const wide = voice === "water";
    const drift = (Math.random() - 0.5) * (wide ? 14 : 6); // cents — hand-wound
    const parts: [number, number, number, number][] = [
      [1, 1, dur, drift], // fundamental
      [1, 0.5, dur * 0.9, drift + (wide ? -9 : 3.5)], // detuned double
    ...
    this.burst({
      ...
      dur: 0.005,
      peak: peak * 0.2,
      filter: { type: "bandpass", freq: voice === "low" ? 3000 : 5000, q: 1 },
    });
    ...
    for (const [ratio, amp, dec, cents] of parts) {
      ...
      // water: every partial sinks as it rings — the note drowns
      if (wide) osc.frequency.exponentialRampToValueAtTime(freq * ratio * 0.985, t0 + dec);
  ...
  private scheduleThemeScore(delayMs: number): void {
    const id = window.setTimeout(() => {
      ...
      this.themeBass(c.bass, t0, BAR, def.bassPulse);
      const spread = [-0.55, 0.35, -0.2, 0.5, -0.4];
      c.pad.forEach((f, i) => {
        this.themePadVoice(f, spread[i % spread.length]!, t0, BAR, 0.045, def.padStyle);
      });
      ...
      if (def.toll !== null && this.themeCycle % def.toll.everyBars === 2) {
        this.themeBell(def.toll.freq, 2 + Math.random() * 4, 0.016, 0.45, 1.4, 6, "box");
      }
      this.themeCycle++;
